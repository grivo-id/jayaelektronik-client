...
function SidebarBlogMenuItem({ className, item, depth = 0, lang }: any) {
  const { t } = useTranslation(lang, 'common');
  const pathname = usePathname();
  const searchParams = useSearchParams();
  const { updateQueryparams } = useQueryParam(pathname ?? '/');
  const [formState, setFormState] = useState<string>('');
  const slug = convertToSlug(item.blog_category_name);
  ...
  useEffect(() => {
    updateQueryparams('categoryBlogs', formState.toString());
            ...
            {items?.map((currentItem) => {
              ...
              return (
                <SidebarBlogMenuItem
                  ...
                  key={`${currentItem.blog_category_name}${currentItem.blog_category_id}`}
                  item={currentItem}
                  depth={childDepth}
